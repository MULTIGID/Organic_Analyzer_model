...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the 38-class PlantVillage model.")
    ...
    model.eval()
    _, _, loader, classes = create_plantvillage_loaders(
    ...
    with torch.inference_mode():
        for images, targets in tqdm(loader, desc="Testing PlantVillage"):
    ...
    metrics.update({
        "images": len(labels),
        "milliseconds_per_image": (time.perf_counter() - started) * 1000 / max(len(labels), 1),
